...
transformed parameters{
  // matrix[nobs+nsims,nobs+nsims] Sigma_z = cov_exp_quad(to_array_1d(z),to_array_1d(z), alpha,rho);
  // matrix[nobs,nobs] Sigma_y = sigma2*diag_matrix(rep_vector(1,nobs));
  // matrix[nobs,nobs+nsims] Sigma_y2 = append_col(Sigma_y,rep_matrix(0,nobs,nsims));
  // matrix[nobs+nsims,nobs+nsims] Sigma_y3 = append_row(Sigma_y2,rep_matrix(0,nsims,nobs+nsims));
  // Sigma_z = Sigma_z + Sigma_y3;
...
model {
  ...
  sigma2 ~ normal(0,5); // gamma (shape, rate)
