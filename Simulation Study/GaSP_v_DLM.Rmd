...
For the following experiments, I consider a scenario to compare the standard GaSP formulation with the DLM-GaSP. This comparison considers 10 training runs of a differential equation computer simulation over a short time horizon of 20 days. I generate 2000 MCMC samples, with the first 1000 discarded as warm up. Priors on the calibration parameters are flat uniform(0,1). I've found that my vanilla GaSP Stan code becomes almost unusable for $T>0$ and more than 10 training runs, though the DLM-GaSP scales very well for long time series and many more training runs. This shows the applicability to spatio-temporal data. 

## Training Runs

### Training Runs for GaSP Model
...
paste("DLM-GaSP Run Time: ",round(end_time2-start_time2,2), "seconds")
```
